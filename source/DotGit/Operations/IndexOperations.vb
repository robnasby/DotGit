...
#Region "Methods"
    ...
    ''' <summary>
    ''' Remove a file from the index of a repository.
    ''' </summary>
    ''' <param name="path">
    ''' The path to the file in the repository.
    ''' </param>
    ''' <param name="force">
    ''' Force remove the file, even if it could result in lost data.
    ''' </param>
    ...
    ''' Force remove the file, even if it could result in lost data.
    ''' </param>
    Public Sub Remove(paths As IEnumerable(Of String),
               Optional force As Boolean = False)

        Dim arguments As String = String.Format("--cached -- {0}", String.Join(" ", paths.Select(Function(p As String) String.Format("""{0}""", p))))
        If force Then arguments = "-f " + arguments
        Dim command As New Command("rm", commandArguments:=arguments, repositoryPath:=Me.Repository.Path)
        command.Execute()

        If Not command.Status = CommandStatusOption.SUCCEEDED Then _
            Throw New ApplicationException("Unable to remove files from the index." + Environment.NewLine +
                                           String.Join(Environment.NewLine, paths.Select(Function(p As String) String.Format("'{0}'", p))))
